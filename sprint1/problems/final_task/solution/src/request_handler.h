...
public:
    ...
    void operator()(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        ...
        if (target.starts_with("/api/")) {
            send(HandleApiRequest(std::move(req), target));
        } else {
            // Не API запрос — Bad Request
...
private:
    // Формирует JSON тело ошибки
    static std::string MakeErrorBody(std::string_view code, std::string_view message) {
        json::object obj;
        obj["code"] = code;
        obj["message"] = message;
    ...
    // Перегрузка без шаблонных параметров для удобства
    static http::response<http::string_body> MakeJsonResponse(
            http::status status,
            std::string body,
            unsigned version,
    ...
    http::response<http::string_body> HandleApiRequest(
        ...
        // Любой другой /api/... запрос — Bad Request
