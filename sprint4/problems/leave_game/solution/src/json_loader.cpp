...
model::Game LoadGame(const std::filesystem::path& json_path, bool randomize_spawn_points,
    ...
    double retirement_time_seconds = 60.0;
    if (obj.contains("dogRetirementTime")) {
        retirement_time_seconds = obj.at("dogRetirementTime").as_double();
    }

    model::Game game(randomize_spawn_points, loot_period, loot_probability);
    game.SetRetirementTime(std::chrono::milliseconds(
        static_cast<int64_t>(retirement_time_seconds * 1000)));
