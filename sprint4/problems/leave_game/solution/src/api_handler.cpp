...
std::optional<int> ParseQueryParam(std::string_view query, std::string_view key) {
    size_t pos = query.find(key);
    while (pos != std::string_view::npos) {
        size_t key_end = pos + key.size();
        if (key_end < query.size() && query[key_end] == '=') {
            size_t value_start = key_end + 1;
            size_t value_end = query.find('&', value_start);
            std::string_view value = query.substr(
                value_start, value_end == std::string_view::npos ? std::string_view::npos
...
StringResponse ApiHandler::HandleApiRequest(const StringRequest& req) {
    ...
    if (target.starts_with("/api/v1/game/records")) {
        return HandleRecordsRequest(req, target);
    }
...
StringResponse ApiHandler::HandleStateRequest(const StringRequest& req) {
    ...
    return ExecuteAuthorized(req, [&](app::Player& player) {
	    ...
	    for (const auto& item : dog_ptr->GetBag()) {
	        json::object bag_item;
	        bag_item["id"] = item.id;
	        bag_item["type"] = item.type;
	        bag_array.push_back(std::move(bag_item));
	    }
	    entry["bag"] = std::move(bag_array);
            entry["score"] = dog_ptr->GetScore();
...
StringResponse ApiHandler::HandleRecordsRequest(const StringRequest& req, std::string_view target) {
    unsigned version = req.version();
    bool keep_alive = req.keep_alive();
    ...
    size_t q_pos = target.find('?');
    if (q_pos != std::string_view::npos) {
        query = target.substr(q_pos + 1);
    }
