...
public:
    ...
    const app::Players& GetPlayers() const noexcept {
        return players_;
    }

    const app::PlayerTokens& GetTokens() const noexcept {
        return tokens_;
    }
    ...
    void RestorePlayer(const Token& token, model::Dog::Id dog_id, const std::string& map_id_str) {
        model::Map::Id map_id{map_id_str};
        model::GameSession& session = game_.GetOrCreateSessionByMapId(map_id);
        ...
        Player& player = players_.Add(dog, &session);
        tokens_.AddPlayerWithToken(token, player);
    }
