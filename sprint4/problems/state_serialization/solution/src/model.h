...
public:
    ...
    model::Dog::Id GetNextDogId() const noexcept {
        return next_dog_id_;
    }
    ...
    void SetLostObjects(std::vector<LostObject> objects) {
...
public:
    ...
    const std::vector<std::unique_ptr<GameSession>>& GetSessions() const noexcept {
        return sessions_;
    }

    GameSession& GetOrCreateSessionByMapId(const Map::Id& map_id) {
        return FindOrCreateSession(map_id);
    }
